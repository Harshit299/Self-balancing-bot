# PID gains
Kp_angle = 42.6  # Proportional gain for the angle
Ki_angle = 930.7  # Integral gain for the angle
...
def sysCall_actuation():
    ...
    if (message == sim.message_keypress):
        if (data[0] == 2008):  # forward (up arrow)
            ...
            error_angle = eq_angle - angle  # We want the tilt angle to be 0 (upright position)
    ...
    # Store current error for the next derivative calculation
            ...
            sim.setJointTargetVelocity(left_joint, control + 0.2)
            sim.setJointTargetVelocity(right_joint, control + 0.2)
    ...
    # Store current error for the next derivative calculation
            ...
            sim.setJointTargetVelocity(left_joint, control + 2.5)
            sim.setJointTargetVelocity(right_joint, control - 2.5)
    ...
    # Store current error for the next derivative calculation
            ...
            if total_error > 0.05 or total_error < -0.05:
